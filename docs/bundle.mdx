---
title: "Bundle"
sidebarTitle: "Bundle files"
description: "Declare groups of module instances and their values in a single CUE file."
---
...
The version number must follow [Timoni's semantic versioning](/cue/module/publishing#version-format).
...
to a field to set its value from the [Runtime](/bundle-runtime).
...
for more details please see the [Bundle Runtime documentation](/bundle-runtime).
...
- Custom resources that are not kstatus-compliant, for which the module
  defines [custom health checks](/cue/module/health-checks)
...
To update the module versions and digests referenced in a bundle,
you can use the `timoni bundle update` command. The update policy
is declared per module reference with the `@timoni(update:...)` attribute:
...
For the update policies, the `--level` flag and the CI integration,
see [Bundle updates](/bundle-update).
